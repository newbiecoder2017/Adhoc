...
def pull_data(s):
    ...
    return pdr.get_data_yahoo(s, start="2000-12-31", end="2019-12-31")['Adj Close']
...
if __name__ == "__main__":
    ...
    universe_list = ['XLRE', 'XLB', 'XLI', 'XLY', 'XLP', 'XLE', 'XLF', 'XLU', 'XLV', 'XLK', 'XLC', 'BIL', 'SHY','^SP500TR']

    # Universe Adj.Close dataframe
    # df = pd.DataFrame({s:pull_data(s) for s in universe_list})
    # df.rename(columns={'^SP500TR': 'SPY'}, inplace=True)
